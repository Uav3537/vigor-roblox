...
function createNetworkClients(opts) {
  ...
  const usersApi = import_vigor_fetch2.vigor.fetch("https://users.roblox.com/v1").middlewares(credentialMiddlewares(true)).settings((s) => s.unretryStatus(429).maxRestarts(CREDENTIAL_MAX_RESTARTS)).retry(
    (r) => r.settings((s) => s.maxAttempts(7)).algorithms((a) => a.backoff({ initial: 200, unit: 800, multiplier: 1.7 }))
  );
