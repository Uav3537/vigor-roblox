...
function makeCredentialMiddlewares(opts) {
  ...
  return import_vigor_fetch.vigor.builders.fetch.middlewares().before("intercept", async (ctx, api) => {
...
function createNetworkClients(opts) {
  ...
  const usersPlainApi = import_vigor_fetch2.vigor.fetch("https://users.roblox.com/v1").retry(
    (r) => r.settings((s) => s.maxAttempts(7)).algorithms((a) => a.backoff({ initial: 200, unit: 800, multiplier: 1.7 }))
  );
  const usersApi = import_vigor_fetch2.vigor.fetch("https://users.roblox.com/v1").middlewares(credentialMiddlewares({ winInet: true })).settings((s) => s.unretryStatus(429).maxRestarts(CREDENTIAL_MAX_RESTARTS)).retry(
    (r) => r.settings((s) => s.maxAttempts(7)).algorithms((a) => a.backoff({ initial: 200, unit: 800, multiplier: 1.7 }))
  );
  ...
  const gamesApi = import_vigor_fetch2.vigor.fetch("https://games.roblox.com/v1").middlewares(credentialMiddlewares()).settings((s) => s.unretryStatus(429).maxRestarts(CREDENTIAL_MAX_RESTARTS)).retry(
    (r) => r.settings((s) => s.maxAttempts(5)).algorithms((a) => a.backoff({ initial: 1e3, multiplier: 2.5 }))
  );
  const gamesServersApi = import_vigor_fetch2.vigor.fetch("https://games.roblox.com/v1").middlewares(credentialMiddlewares({ allowOAuth: false })).settings((s) => s.unretryStatus(429).maxRestarts(CREDENTIAL_MAX_RESTARTS)).retry(
    (r) => r.settings((s) => s.maxAttempts(5)).algorithms((a) => a.backoff({ initial: 1e3, multiplier: 2.5 }))
  );
  ...
  function buildPresenceApi(cookie) {
    ...
    return import_vigor_fetch2.vigor.fetch("https://presence.roblox.com/v1").middlewares(middlewares).settings((s) => s.unretryStatus(429).maxRestarts(CREDENTIAL_MAX_RESTARTS)).retry(
