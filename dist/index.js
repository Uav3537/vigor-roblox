...
function createNetworkClients(opts) {
  ...
  function buildGamejoinApi(cookie) {
    ...
    return vigor2.fetch("https://gamejoin.roblox.com/v1").middlewares(middlewares).retry(
...
import { z } from "zod";
var RobloxUserIdSchema = z.number().int().positive().brand();
var RobloxUserNameSchema = z.string().trim().min(1).brand();
var RobloxDisplayNameSchema = z.string().min(1).brand();
var RobloxCookieSchema = z.string().min(50).startsWith(
  "_|WARNING:-DO-NOT-SHARE-THIS.--Sharing-this-will-allow-someone-to-log-in-as-you-and-to-steal-your-ROBUX-and-items.|_",
  { message: "Not a valid .ROBLOSECURITY cookie" }
).brand();
var RobloxPlaceIdSchema = z.number().int().positive().brand();
var RobloxUniverseIdSchema = z.number().int().positive().brand();
var RobloxJobIdSchema = z.string().uuid("JobId must be a valid UUID").brand();
var RobloxAssetIdSchema = z.number().int().positive().brand();
  ...
  async function fetchPresenceRaw(userIds, client) {
    const grouped = await vigor5.all(
      ...chunk(userIds, 50).map(
        (group) => () => client.path("presence", "users").body("overwrite", { userIds: group }).middlewares(pickKeyValidated("userPresences", z5.array(RobloxPresenceEntrySchema))).request()
      )
    ).request();
    return grouped.flat();
  }
